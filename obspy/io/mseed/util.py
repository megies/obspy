...
def get_timing_and_data_quality(*args, **kwargs):
    # Wrapper around new and more general function.
    _r = get_flags(activity_flags=False, io_flags=False,
                   data_quality_flags=True, timing_quality=True,
                   *args, **kwargs)
    results = {}

    results["data_quality_flags"] = list(_r["data_quality_flags"].values())

    tq = _r["timing_quality"]
    if len(tq["all_values"]):
        results['timing_quality_upper_quantile'] = tq["upper_quartile"]
        results['timing_quality_min'] = tq["min"]
        results['timing_quality_lower_quantile'] = tq["lower_quartile"]
        results['timing_quality_average'] = tq["mean"]
        results['timing_quality_median'] = tq["median"]
        results['timing_quality_max'] = tq["max"]

    return results


def get_flags(file_or_file_object, starttime=None, endtime=None,
              io_flags=True, activity_flags=True, data_quality_flags=True,
              timing_quality=True):
    """
