...
do
    ...
    for FILE in `find debian/tmp -type f -regex ".*/python3.*/*-packages/obspy/.*/tests/${DIRTYPE}/.*"` `find debian/tmp -type f -regex ".*/python3.*/*-packages/obspy/io/mseed/src/libmseed/test/data/.*"`
    do
        SOURCE=`echo $FILE | sed 's#.*-packages/obspy/#usr/share/obspy/#'`
        DESTINATION=`echo $FILE | sed 's#debian/tmp/##'`
